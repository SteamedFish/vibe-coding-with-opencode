...
> 💡 **关于插件自动安装**：`plugin` 数组中的插件在 OpenCode 启动时自动安装。使用 `"oh-my-opencode@latest"` 格式会在每次启动时检查更新，但**会拖慢启动速度**。建议日常不带 `@latest`，需要更新时手动修改。
...
> ⚠️ **Superpowers 需要独立安装**，不随 oh-my-opencode 一同安装。
...
> ⚠️ 如果你的订阅不在列表中（如 MiniMax、Kimi 等），可以**随便选一个**完成安装，然后手动编辑配置文件 `~/.config/opencode/oh-my-opencode.json`，把模型名改成你实际可用的模型即可。
...
**配置文件示例**（`~/.config/opencode/oh-my-opencode.json`）：
...
这些 Category 在 `oh-my-opencode.json` 的 `categories` 字段中配置（见前面安装配置部分的示例）。Sisyphus 根据 IntentGate 的分类结果自动选择模型——你不需要手动切换。
...
GSD 是另一个流行的工作流插件，使用**阶段制（Phase-based）**方式管理项目：

```
讨论 (Discuss) → 计划 (Plan) → 执行 (Execute) → 验证 (Verify) → 完成 (Complete)
                                      ↑                  |
                                      └── 未通过 ────────┘
```
...
| 命令 | 作用 |
|------|------|
| `/gsd-new-project` | 初始化 GSD 项目 |
| `/gsd-discuss-phase` | 讨论阶段需求 |
| `/gsd-plan-phase` | 制定阶段计划 |
...
**Spec-Kit**（[github/spec-kit](https://github.com/github/spec-kit)）是 GitHub 官方推出的规范驱动开发工具包，采用 Python 编写。它提供了一种**基于项目宪法（AGENTS.md）的分阶段工作流**。

**核心理念**：
通过 `AGENTS.md` 定义项目规范（宪法），然后使用命令行工具将规范转化为可执行的计划和任务。每个阶段都有明确的输入、输出和验收标准。

**工作流程**：

```
/speckit.constitution → /speckit.specify → /speckit.plan → /speckit.tasks → /speckit.implement
  生成项目宪法           细化需求规格          制定计划           拆解任务           执行实现
```
...
| 特点 | 说明 |
|------|------|
| **宪法驱动** | 以 `AGENTS.md` 为项目根本法，所有行为基于规范 |
| **扩展丰富** | 70+ 社区扩展，覆盖文档、代码、流程、集成等 |
| **模板系统** | 内置多种项目模板，快速启动 |
| **多 Agent 支持** | 与 Claude、Codex、Copilot 等多种 AI 工具集成 |
...
```bash
# 使用 uv（推荐）
uv tool install git+https://github.com/github/spec-kit
...
| 维度 | OMO | GSD | Spec-Kit |
|------|-----|-----|----------|
| **方法论** | Agent 协作 + 智能调度 | 阶段制（Phase-based） | 宪法驱动 |
| **核心机制** | IntentGate + Category 委派 | 讨论→计划→执行→验证→完成 | AGENTS.md → Specify → Plan → Tasks → Implement |
| **自动化程度** | 高（自动选择策略和模型） | 中等（按阶段推进） | 中等（按规范执行） |
| **适用规模** | 中大型项目 | 中大型项目 | 大型团队项目 |
| **团队要求** | 需要了解 Agent 角色 | 需要遵循阶段流程 | 需要维护 AGENTS.md 规范 |
| **与 OpenCode 集成** | 原生插件 | 原生插件 | 命令行工具 |

> 💡 **选择建议**：
> - 如果需要**智能 Agent 协作和自动模型调度**，选择 **OMO**
> - 如果需要**清晰的阶段管控和进度跟踪**，选择 **GSD**
> - 如果需要**严格的规范约束和标准化流程**，选择 **Spec-Kit**
...
如果你使用 oh-my-opencode（OMO），它会**默认安装以下三个 MCP 服务**。这三个服务极大扩展了 AI 的能力边界，即使你不使用 OMO，也强烈建议手动安装它们：
